...
/* Modified for LinuxUsbDisplay Ubuntu compatibility, 2026-09-08.
 * Original source and local changes are documented in ../UPSTREAM.md.
 */
...
static void ms912x_put_dma_device(void *data)
{
	put_device(data);
}
#endif

static void ms912x_destroy_workqueue(struct drm_device *dev, void *data)
{
			    ...
			    const struct usb_device_id *id)
{
	...
	if (dma_dev) {
#ifdef MS912X_HAS_DMA_DEV
		drm_dev_set_dma_dev(dev, dma_dev);
		put_device(dma_dev);
#else
		ms912x->dmadev = dma_dev;
		ret = devm_add_action_or_reset(&interface->dev,
					       ms912x_put_dma_device, dma_dev);
		if (ret)
			return ret;
#endif
	} else {
	...
	ms912x->workqueue = alloc_ordered_workqueue(DRIVER_NAME, 0);
	if (!ms912x->workqueue)
		return -ENOMEM;
	ret = drmm_add_action_or_reset(dev, ms912x_destroy_workqueue,
				      ms912x->workqueue);
	if (ret)
		return ret;
...
#elif defined(MS912X_HAS_FBDEV_SHMEM)
